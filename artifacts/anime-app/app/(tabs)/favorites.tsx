    ...
    <View style={[styles.container, { paddingTop: topPad }]}>
      ...
      <View style={styles.header}>
        ...
        {favorites.length > 0 && (
          <View style={styles.countBadge}>
            <LinearGradient colors={[Colors.primary, Colors.secondary]} style={styles.countGrad}>
      ...
      {favorites.length === 0 ? (
        <View style={styles.empty}>
          ...
          <Pressable style={styles.emptyHint}>
            <Feather name="compass" size={14} color={Colors.primary} />
            <Text style={styles.emptyHintText}>Explorar anime</Text>
          </Pressable>
        </View>
      ) : (
        <FlatList
...
const styles = StyleSheet.create({
  ...
  countBadge: { borderRadius: 14, overflow: "hidden" },
  countGrad: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 14 },
  countText: { color: "#fff", fontSize: 14, fontWeight: "900" },

  listHeader: { marginBottom: 14 },
  listHeaderLine: { height: 1, backgroundColor: Colors.border, borderRadius: 1 },
  list: { padding: 16, paddingTop: 8, gap: 14 },
  ...
  emptyHintText: { color: Colors.primary, fontSize: 13, fontWeight: "700" },
});
