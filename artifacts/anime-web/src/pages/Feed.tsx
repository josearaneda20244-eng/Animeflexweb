...
function statusLabel(status: string): { text: string; color: string; icon: React.ReactNode } {
  switch (status) {
    case "watching":     return { text: "está viendo",      color: "#6C63FF", icon: <Tv2 size={13} /> };
    ...
    default:             return { text: "actualizó",        color: "#A78BFA", icon: <Rss size={13} /> };
  }
}
    ...
    <div style={{ minHeight: "100vh", background: "#090A12" }}>
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "72px 16px 48px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 24 }}>
          <div style={{ width: 4, height: 24, borderRadius: 2, background: "linear-gradient(180deg,#6C63FF,#EC4899)" }} />
          <Rss size={20} color="#6C63FF" />
          <h1 style={{ color: "#F1F1F5", fontSize: 22, fontWeight: 900, margin: 0 }}>Feed de actividad</h1>
        </div>

        {!token ? (
          <div style={{ background: "#13131C", borderRadius: 20, padding: "48px 24px", textAlign: "center", border: "1px solid rgba(255,255,255,0.07)" }}>
            ...
            <button
              onClick={() => navigate("/")}
              style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "10px 20px", borderRadius: 12, background: "linear-gradient(135deg,#6C63FF,#4F46E5)", border: "none", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
              Ir al inicio
            </button>
          </div>
        ) : loading ? (
          <div style={{ textAlign: "center", padding: "80px 0" }}>
            <Loader2 size={28} color="rgba(108,99,255,0.5)" style={{ margin: "0 auto", animation: "spin 1s linear infinite" }} />
          </div>
        ) : activities.length === 0 ? (
          <div style={{ background: "#13131C", borderRadius: 20, padding: "48px 24px", textAlign: "center", border: "1px solid rgba(255,255,255,0.07)" }}>
            <UserPlus size={48} color="rgba(108,99,255,0.3)" style={{ margin: "0 auto 16px", display: "block" }} />
            ...
            {activities.map((a, i) => {
              ...
              return (
                <div key={`${a.user_id}-${a.anime_id}-${i}`}
                  style={{ background: "#13131C", borderRadius: 14, padding: 14, border: "1px solid rgba(255,255,255,0.06)", display: "flex", gap: 12, alignItems: "center", cursor: "pointer" }}
