...
function statusLabel(status: string): { text: string; color: string; icon: React.ReactNode } {
  switch (status) {
    case "watching":     return { text: "está viendo",      color: "#DC2626", icon: <Tv2 size={13} /> };
    ...
    default:             return { text: "actualizó",        color: "#FECACA", icon: <Rss size={13} /> };
  }
}
    ...
    <div style={{ minHeight: "100vh", background: "#000" }}>
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "72px 16px 48px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 24 }}>
          <div style={{ width: 4, height: 24, borderRadius: 2, background: "linear-gradient(180deg,#DC2626,#DC2626)" }} />
          <Rss size={20} color="#DC2626" />
        ...
        {!token ? (
          <div style={{ background: "#0a0a0a", borderRadius: 20, padding: "48px 24px", textAlign: "center", border: "1px solid rgba(255,255,255,0.07)" }}>
            ...
            <button
              onClick={() => navigate("/")}
              style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "10px 20px", borderRadius: 12, background: "linear-gradient(135deg,#DC2626,#991B1B)", border: "none", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
              Ir al inicio
            </button>
          </div>
        ) : loading ? (
          <div style={{ textAlign: "center", padding: "80px 0" }}>
            <Loader2 size={28} color="rgba(220,38,38,0.5)" style={{ margin: "0 auto", animation: "spin 1s linear infinite" }} />
          </div>
        ) : activities.length === 0 ? (
          <div style={{ background: "#0a0a0a", borderRadius: 20, padding: "48px 24px", textAlign: "center", border: "1px solid rgba(255,255,255,0.07)" }}>
            <UserPlus size={48} color="rgba(220,38,38,0.3)" style={{ margin: "0 auto 16px", display: "block" }} />
                ...
                <div key={`${a.user_id}-${a.anime_id}-${i}`}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <span
                          style={{ color: "#FECACA", fontSize: 13, fontWeight: 700, cursor: "pointer" }}
