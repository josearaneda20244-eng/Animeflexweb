import { useQuery } from "@tanstack/react-query";
  ...
  /* ── NEWS SECTION (Solo Leveling SISTEMA aesthetic) ── */
  interface NewsItem {
    title: string;
    link: string;
    description: string;
  ...
  function NewsSection() {
    ...
    return (
      <div style={{ marginTop: 36, position: "relative" }}>
        ...
        <div style={{ position: "relative", padding: "0 18px 14px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6, fontFamily: "'JetBrains Mono', 'Courier New', ui-monospace, monospace" }}>
              <motion.span animate={{ opacity: [1, 0.25, 1] }} transition={{ duration: 1.4, repeat: Infinity }}
                style={{ width: 7, height: 7, background: "#DC2626", boxShadow: "0 0 10px #DC2626", display: "inline-block" }} />
              <span style={{ color: "#DC2626", fontSize: 10, letterSpacing: 4, fontWeight: 900, textShadow: "0 0 10px rgba(220,38,38,0.6)", textTransform: "uppercase" }}>
                [ NOTICIAS · ANIMEFLEX OS ]
              </span>
              <span style={{ background: "linear-gradient(135deg,#DC2626,#F97316)", color: "#fff", fontSize: 9, fontWeight: 900, letterSpacing: 2, padding: "2px 7px", borderRadius: 3, boxShadow: "0 0 12px rgba(220,38,38,0.55)" }}>
                NEW
              </span>
            </div>
            <h2 style={{ color: "#F1F1F5", fontSize: 22, fontWeight: 900, margin: 0, letterSpacing: -0.4, textShadow: "0 0 18px rgba(220,38,38,0.25)" }}>
              📰 Últimas noticias del mundo anime
            </h2>
            <div style={{ color: "rgba(255,255,255,0.4)", fontSize: 12, marginTop: 4, fontFamily: "'JetBrains Mono', 'Courier New', ui-monospace, monospace", letterSpacing: 0.5 }}>
              &gt; Feed agregado de Kudasai · ANMTV · Ramen Para Dos
            </div>
          </div>
        </div>

        <ScrollableCarousel scrollAmount={620}>
          {isLoading
            ? Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="skeleton" style={{ width: 290, height: 320, borderRadius: 4, flexShrink: 0, border: "1px solid rgba(220,38,38,0.15)" }} />
              ))
            : news.map((n, i) => {
                const colorPalette = ["#DC2626", "#F97316", "#A78BFA", "#22C55E"];
                const accent = colorPalette[i % colorPalette.length];
                return (
                  <motion.a
                    key={`${n.link}-${i}`}
                    href={n.link}
                    ...
                    whileHover={{ y: -4 }}
                    className="news-card"
                    style={{ width: 290, flexShrink: 0, textDecoration: "none", color: "inherit", ["--accent" as any]: accent }}
                  >
                    {/* Media */}
                    <div className="news-card__media">
                      {n.image ? (
                        ...
                        <div className="news-card__media-fallback">📰</div>
                      )}
        ...
        <style>{`
          ...
          .news-card:hover {
            border-color: var(--accent);
            box-shadow: 0 18px 50px rgba(0,0,0,0.55), 0 0 24px var(--accent), inset 0 0 24px rgba(220,38,38,0.06);
          }
          .news-card__media {
            position: relative;
            width: 100%;
          ...
          .news-card__media-fallback {
            ...
            background: radial-gradient(circle at 50% 50%, rgba(220,38,38,0.25), transparent 70%);
          }
          .news-card__overlay {
            position: absolute; inset: 0;
      ...
      <div style={{ minHeight: "100vh" }}>
        ...
        <div style={{ paddingTop: 56 }}>
          ...
          <LazySection minHeight={380}>
            {/* News Feed (Solo Leveling SISTEMA aesthetic) */}
            <NewsSection />
            <SectionDivider />
          </LazySection>
