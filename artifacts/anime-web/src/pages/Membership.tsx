...
export default function Membership() {
  ...
  async function handleApprove(subscriptionId: string) {
    ...
    setError(null);
    try {
      await apiClient.post("/api/membership", { action: "activate", subscriptionId });
      await refreshUser();
      setSuccess(true);
    } catch (e: any) {
  ...
  async function handleManage() {
    setLoading(true);
    try {
      const data = await apiClient.post<{ url: string }>("/api/membership", { action: "portal" });
          ...
          <div className="relative bg-[#12122a] border-2 border-purple-500 rounded-2xl p-8 shadow-xl shadow-purple-900/30">
            ...
            <ul className="space-y-3 mb-8 text-sm">
              {[
                ...
                { text: "Sin anuncios en toda la plataforma" },
                ...
                {!loading && PAYPAL_CLIENT_ID && PAYPAL_PLAN_ID ? (
                  ...
                  !loading && (
                    <div className="text-center text-yellow-500 text-sm py-4 bg-yellow-900/20 rounded-xl border border-yellow-700/40 p-4">
                      ⚙️ PayPal aún no está configurado.
                      <br />
                      <span className="text-gray-400 text-xs mt-1 block">
                        Añade VITE_PAYPAL_CLIENT_ID y VITE_PAYPAL_PLAN_ID en Vercel.
                      </span>
                    </div>
                  )
                )}
