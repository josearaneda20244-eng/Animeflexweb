...
const AVATAR_COLORS = ["#7C6FFF", "#EC4899", "#22C55E", "#F59E0B", "#06B6D4", "#EF4444", "#8B5CF6"];
...
function ReplyThread({ animeId, parentId, parentAuthor, onClose, onReplyCountChange }: {
  ...
  return (
    <div style={{ marginTop: 8, marginLeft: 40, borderLeft: "2px solid rgba(124,111,255,0.2)", paddingLeft: 12 }}>
      {loading ? (
        <div style={{ padding: "8px 0" }}>
          <Loader2 size={14} color="rgba(124,111,255,0.5)" style={{ animation: "spin 1s linear infinite" }} />
        </div>
      ) : (
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 6, marginTop: 5 }}>
              ...
              <button type="submit" disabled={!replyText.trim() || posting}
                style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 12px", borderRadius: 7, background: replyText.trim() && !posting ? "linear-gradient(135deg,#7C6FFF,#5B52F5)" : "rgba(255,255,255,0.07)", border: "none", color: replyText.trim() && !posting ? "#fff" : "rgba(255,255,255,0.25)", fontSize: 11, fontWeight: 700, cursor: replyText.trim() && !posting ? "pointer" : "not-allowed" }}>
    ...
    <div style={{ marginTop: 32, padding: "0 0 16px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ width: 3, height: 20, borderRadius: 2, background: "#7C6FFF" }} />
          <MessageCircle size={16} color="#7C6FFF" />
        ...
        {comments.length > 1 && (
          <div style={{ display: "flex", background: "#0F0F1A", borderRadius: 10, overflow: "hidden", border: "1px solid rgba(255,255,255,0.08)" }}>
      ...
      {user ? (
        <form onSubmit={handleSubmit} style={{ background: "#0F0F1A", borderRadius: 16, padding: 16, marginBottom: 20, border: "1px solid rgba(255,255,255,0.07)" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 10 }}>
            <label style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer", userSelect: "none" }}>
              <div onClick={() => setIsSpoiler((v) => !v)}
                style={{ width: 32, height: 18, borderRadius: 9, background: isSpoiler ? "#7C6FFF" : "rgba(255,255,255,0.1)", position: "relative", cursor: "pointer", transition: "background 0.2s" }}>
              ...
              <span style={{ color: isSpoiler ? "#B39DFF" : "rgba(255,255,255,0.4)", fontSize: 12, fontWeight: 600 }}>Spoiler</span>
            </label>
            <button type="submit" disabled={!text.trim() || posting}
              style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 10, background: text.trim() && !posting ? "linear-gradient(135deg,#7C6FFF,#5B52F5)" : "rgba(255,255,255,0.07)", border: "none", color: text.trim() && !posting ? "#fff" : "rgba(255,255,255,0.25)", fontSize: 13, fontWeight: 700, cursor: text.trim() && !posting ? "pointer" : "not-allowed" }}>
        ...
        <div style={{ background: "#0F0F1A", borderRadius: 16, padding: 16, marginBottom: 20, border: "1px solid rgba(255,255,255,0.07)", textAlign: "center", color: "rgba(255,255,255,0.4)", fontSize: 13 }}>
      ...
      {loading ? (
        <div style={{ textAlign: "center", padding: "32px 0" }}>
          <Loader2 size={24} color="rgba(124,111,255,0.5)" style={{ margin: "0 auto", animation: "spin 1s linear infinite" }} />
        </div>
      ) : sorted.length === 0 ? (
        <div style={{ textAlign: "center", padding: "32px 0" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {sorted.map((c) => {
            ...
            return (
              <div key={c.id} style={{ background: "#0F0F1A", borderRadius: 14, padding: 14, border: "1px solid rgba(255,255,255,0.06)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                  ...
                  {token && (
                    <button onClick={() => { setReplyingTo(isReplying ? null : c.id); if (!repliesExpanded && !isReplying) setExpandedReplies((p) => new Set([...p, c.id])); }}
                      style={{ display: "flex", alignItems: "center", gap: 5, padding: "4px 10px", borderRadius: 8, background: isReplying ? "rgba(124,111,255,0.12)" : "rgba(255,255,255,0.05)", border: `1px solid ${isReplying ? "rgba(124,111,255,0.3)" : "rgba(255,255,255,0.08)"}`, color: isReplying ? "#B39DFF" : "rgba(255,255,255,0.4)", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
                      <Reply size={11} />
                      Responder
                    </button>
                  )}
                  ...
                  {c.reply_count > 0 && (
                    <button onClick={() => toggleReplies(c.id)}
                      style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 10px", borderRadius: 8, background: repliesExpanded ? "rgba(124,111,255,0.08)" : "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", color: "rgba(255,255,255,0.4)", cursor: "pointer", fontSize: 11, fontWeight: 600 }}>
